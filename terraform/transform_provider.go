...
func (t *CloseProviderTransformer) Transform(g *Graph) error {
	...
	cpm := make(map[string]bool)
	var err error

	for _, v := range pm {
		p := v.(GraphNodeProvider)

		// see if we already added this
		if cpm[p.ProviderName()] {
			continue
		}

		// close all the providers
		source := &graphNodeCloseProvider{ProviderNameValue: p.ProviderName()}
		g.Add(source)

		cpm[p.ProviderName()] = true

		// connect all the provider's resources to the close node
		for _, s := range g.UpEdges(p).List() {
			g.Connect(dag.BasicEdge(source, s))
		}

		// Close node depends on the provider
		g.Connect(dag.BasicEdge(source, p))
	}
